...
import { apiOriginForOAuth } from "@/lib/auth/api-origin";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    setStoredAuth(accessToken, parseInt(expiresIn, 10));
  ...
  const googleStartUrl = `${apiOriginForOAuth()}/auth/google_oauth2`;
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-12">
      ...
      >
        ...
        <div className="mt-8 flex items-center gap-3">
          <span className="h-px flex-1 border-t border-dotted border-neutral-300 dark:border-neutral-600" aria-hidden />
          <span className="shrink-0 text-xs font-medium uppercase tracking-[0.12em] text-neutral-500 dark:text-neutral-400">
            Or continue with
          </span>
          <span className="h-px flex-1 border-t border-dotted border-neutral-300 dark:border-neutral-600" aria-hidden />
        </div>
        ...
        <a
          href={googleStartUrl}
          className="mt-4 flex h-12 w-full items-center justify-center gap-2.5 rounded-xl border border-neutral-200 bg-white text-sm font-semibold text-neutral-800 shadow-sm transition hover:bg-neutral-50 dark:border-white/15 dark:bg-white/10 dark:text-white dark:hover:bg-white/15"
        >
          <GoogleMark />
          Google
        </a>
        <p className="mt-2 text-center text-[11px] leading-snug text-neutral-400 dark:text-neutral-500">
          Google needs <code className="rounded bg-neutral-100 px-1 dark:bg-white/10">GOOGLE_CLIENT_*</code> on the API. Redirect:{" "}
          <code className="break-all rounded bg-neutral-100 px-1 dark:bg-white/10">{googleStartUrl}/callback</code>
        </p>
